...
namespace dvxplorer_ros_driver
{
	...
	DvxplorerRosDriver::DvxplorerRosDriver(ros::NodeHandle &nh, ros::NodeHandle nh_private) : nh_(nh), imu_calibration_running_(false)
	{
		// load parameters
		...
		{
			reset_sub_ = nh_.subscribe((ns + "/reset_timestamps").c_str(), 1, &DvxplorerRosDriver::resetTimestampsCallback, this);
		}
		...
		{
			timestamp_reset_timer_ = nh_.createTimer(ros::Duration(reset_timestamps_delay), &DvxplorerRosDriver::resetTimerCallback, this);
	...
	void DvxplorerRosDriver::caerConnect()
	{
		...
		// spawn threads
		running_ = true;
		readout_thread_ = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&DvxplorerRosDriver::readout, this)));
	...
	void DvxplorerRosDriver::startCalibrateImu()
	{
	...
	void DvxplorerRosDriver::callback(dvxplorer_ros_driver::DVXplorer_ROS_DriverConfig &config, uint32_t level)
	{
		...
		if (level & (0x01 << 5))
		{
			...
			mean_timestamp_window_ = config.mean_timestamp_window;
			std::cout << "streaming_rate_: " << streaming_rate_ << " max_events_: " << max_events_ << " mean_timestamp_window_: " << mean_timestamp_window_ << std::endl;
		}
	}
				...
				for (int32_t i = 0; i < packetNum; i++)
				{
					...
					if (type == POLARITY_EVENT)
					{
						...
						for (int j = 0; j < numEvents; j++)
						{
							...
							e.ts = reset_time_ + ros::Duration().fromNSec(caerPolarityEventGetTimestamp64(event, polarity) * 1000);
						...
						int mean_timestamp_window = mean_timestamp_window_;

						// throttle event messages
						if ((boost::posix_time::microsec_clock::local_time() > next_send_time) || (streaming_rate == 0) || ((max_events != 0) && (event_array_msg->events.size() > max_events)))
						{
